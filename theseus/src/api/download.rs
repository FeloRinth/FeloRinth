...
async fn download_file(download_url: &str, local_filename: &str, auto_update_supported: bool) -> Result<(), Box<dyn std::error::Error>> {
...
pub async fn init_download(download_url: &str, local_filename: &str, auto_update_supported: bool) {
    println!("[init_download] • Initialize downloading from • {:?}", download_url);
    println!("[init_download] • Save local file name • {:?}", local_filename);
    if let Err(e) = download_file(download_url, local_filename, auto_update_supported).await {
